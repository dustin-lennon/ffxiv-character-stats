...
class ffxivDB
{
    ...
    /**
     * Returns the table prefix for the main site on multisites and the site itself
    ...
    /**
     * Retuns the table with the site (single site installations) or network (multisite)
     * prefix added
     *
     * @param string $table
     * @param bool $applyCaseConversion Whether or not to convert teh table case to what is actually in use
     * @return string
     */
    public static function networkTable($table, $applyCaseConversion = true) {
        if (ffxivSchema::usingLowerCase() && $applyCaseConversion) {
    ...
    /**
     * Returns the table prefix for the given blog ID. On single site installations, this will be equivalent to ffxivDB::networkPrefix().
     *
     * @param int $blogID
     * @return string
     */
    ...
    public function columnExists($table, $col) {
        $table = ffxivDB::networkTable($table);
...
SQL
    ...
    public function truncate($table) { // Ensure everything is deleted if user is using MySQL >= MySQL 5.1.16 and does not have "drop" privileges
...
abstract class ffxivModel {
    ...
    public function update() {
        return $this->getDB()->update($this->getTable(), $this->getData(), [
